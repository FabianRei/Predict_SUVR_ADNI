...
def test_reg(batchSize, testData, test_labels, Net, criterion, dimIn, includePredictionLabels=False, test_eval=True):
    allAccuracy =[]
    allWrongs = []
    predictions = []
    labels = []
    ...
    for batch_idx, (data, target) in enumerate(batch_gen(testData, test_labels, batchSize, shuffle=False)):
        ...
        # Net.eval()
        net_out = Net(data)
        prediction = net_out.detach().cpu().numpy()
        testAcc = list(((prediction[:, 0]-target[:, 0].cpu().numpy())**2))
    ...
    if includePredictionLabels:
        ...
        return np.mean(allAccuracy), np.concatenate((predictions, test_labels), axis=1)
    else:
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(batch_gen(train_data, train_labels, batch_size, shuffle=True)):
            ...
            if is_resnext or is_resnet152:
                ...
                optimizer.step()
                optimizer.zero_grad()
            batch_acc = ((prediction[:, 0].detach()-target[:, 0])**2).cpu().numpy()
        ...
        if epoch % test_interval == 0 and test_interval > 0:
            optimizer.zero_grad()
            test_acc, test_pred_label = test_reg(batch_size, test_data, test_labels, Net, criterion, dim_in, includePredictionLabels=True)
        ...
        apoe_pred_train = np.apply_along_axis(np.argmax, 1, train_predictions[:, 2:])
        train_predictions = np.concatenate((train_predictions[:, :2], apoe_pred_train.reshape(-1, 1)), axis=1)
        # train_target = [float(t) for t in train_target]
        # train_predictions = [float(p) for p in train_predictions]
    return Net, test_acc, test_pred_label, np.mean(epoch_acc), np.mean(loss_arr), np.concatenate((train_predictions, train_target), axis=1)
