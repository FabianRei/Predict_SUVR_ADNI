...
from dl.neural_network.resnet import ResNet50, ResNet50Reg, ResNet50RegMulti
...
from dl.neural_network.train_test_regression_multi import train_reg as train_reg_multi
...
from dl.neural_network.multi_task_loss import MultiTaskLoss, CustomMultiWrapper
...
def train_h5(h5_path, num_epochs=30, label_names=None, extra_info='', lr=0.01, decrease_after=10,
             ...
             use_resnext=False, use_resnet152=False, use_multi_resnet=False, save_model=True, train_by_id = '', extract_features=False,
             threshold=1.11):
    ...
    elif extract_features and not windows_db:
    ...
    if regression:
        ...
        elif use_multi_resnet:
            Net = ResNet50RegMulti(pretrained=pretrained, num_classes=8)
            criterion = CustomMultiWrapper()
            train_func = train_reg_multi
        else:
            Net = ResNet50Reg(pretrained=pretrained, num_classes=1, num_input=num_chan_input)
        if not use_multi_resnet:
    ...
    for i in range(num_epochs):
        ...
        if len(label_names) > 1 and not regression:
            ...
            test_label_acc_test = np.mean((test_pred_label[:, 0] >= threshold) == (test_pred_label[:, 3] >= threshold))
            test_label_acc_train = np.mean((train_pred_label[:, 0] >= threshold) == (train_pred_label[:, 3] >= threshold))
            csv_writer.write_row(test_acc=test_acc, train_acc=train_acc, train_loss=train_loss, epoch=i,
                                 test_label_acc_train=test_label_acc_train, test_label_acc_test=test_label_acc_test)
            print(f"Amyloid status accuracy is {test_label_acc_test * 100:.2f} percent for test and {test_label_acc_train * 100:.2f} percent for train")
        else:
...
if __name__ == '__main__':
    ...
    if not windows_db:
        ...
        train_h5(r'C:\Users\Fabian\stanford\fed_learning\federated_learning_data\slice_data_longitudinal_fixed_multi_no_onehot.h5',
                 pretrained=False, extra_info='', lr=0.001, regression=True, label_names=['multi_res_suvr_age_apoe', 'label_amyloid'],
                 num_epochs=1, seed=1, extract_features=True, batch_size=8, use_multi_resnet=True, threshold=0.79)
